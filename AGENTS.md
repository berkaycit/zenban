...
- `zenban/`: main macOS app target (SwiftUI app entry, Views, ViewModels, Models, Services, Storage, `CmuxImport` host code).
- `zenbanTests/`: unit tests using Swift Testing.
- `zenbanUITests/`: UI tests using XCTest.
- `Vendor/`: vendored native libraries.
- `cmux-import/`: copied `clone/cmux` inputs such as CLI, Ghostty runtime, bonsplit, and bundled assets.
...
- `cmux-import/GhosttyKit.xcframework/`: copied Ghostty runtime from `clone/cmux`.
- `cmux-import/bonsplit/`: copied Swift package for split panes and workspace chrome.
- `cmux-import/ghostty`, `cmux-import/shell-integration`, `cmux-import/terminfo-overlay`, `ghostty/zig-out/share/`: Ghostty assets copied from `clone/cmux` and packaged by the `Copy Ghostty Resources` build phase.
- `cmux-import/bin/`: bundled `cmux`, `claude`, and `open` helpers copied into the app bundle for socket, browser, and Claude integration.
- `zenban/CmuxImport/`: copied cmux workspace host layer (`Workspace`, `TabManager`, panels, browser, notification store, socket controller, AppleScript support).
- `zenban/CmuxHostStore.swift`: Zenban-specific adapter that maps cards onto copied cmux workspaces and browser surfaces.
...
- When changing terminal behavior, verify whether the logic lives in copied `zenban/CmuxImport/` code or Zenban-specific adapter/worktree code (`zenban/CmuxHostStore.swift`, `BoardStore`, `GitService`)
- Don't use emojis
