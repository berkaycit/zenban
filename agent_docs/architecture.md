...
| `TerminalManager` | Card-scoped terminal host manager. Each card owns a cmux-style `Workspace`/`TabManager` tree from `zenban/CmuxParity`, so split layout, browser panels, search state, scrollback, and focus stay alive across card switches. Ghostty runtime/resources plus `Resources/bin/cmux`, `claude`, and `open` are packaged from the cmux import by the `Copy Ghostty Resources` build phase, and socket control now routes through `TerminalController` using cmux workspace/surface environment IDs. |
