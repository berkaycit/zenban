...
| `TerminalManager` | Card-scoped terminal host manager. Each card owns a cmux-style `Workspace`/`TabManager` tree from `zenban/CmuxParity`, and `CardWorkspaceDeckView` keeps only the selected + retiring card mounted during handoff so split layout, browser panels, search state, scrollback, and focus survive card switches without stale Ghostty/WebKit portals bleeding through. Ghostty runtime/resources plus `Resources/bin/cmux`, `claude`, and `open` are packaged from the cmux import by the `Copy Ghostty Resources` build phase, and socket control routes through `TerminalController` using cmux workspace/surface environment IDs. |
