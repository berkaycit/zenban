    ...
    enum InstallerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bundledCLIMissing(let expectedPath):
                return "Bundled Zenban CLI was not found at \(expectedPath)."
            ...
            case .installVerificationFailed(let path):
                return "Installed symlink at \(path) did not point to the bundled Zenban CLI."
...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        refreshGhosttyConfigDrivenShortcuts()
    ...
    @objc func installCmuxCLIInPath(_ sender: Any?) {
        ...
        do {
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.installed", defaultValue: "Zenban CLI Installed"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.installFailed", defaultValue: "Couldn't Install Zenban CLI"),
    ...
    @objc func uninstallCmuxCLIInPath(_ sender: Any?) {
        ...
        do {
            ...
            let prefix = outcome.removedExistingEntry
                ...
                : String(localized: "cli.uninstall.notFound", defaultValue: "No Zenban CLI symlink was found at \(outcome.destinationURL.path).")
            ...
            presentCLIPathAlert(
                title: String(localized: "cli.uninstalled", defaultValue: "Zenban CLI Uninstalled"),
                informativeText: informativeText,
                style: .informational
            )
        } catch {
            presentCLIPathAlert(
                title: String(localized: "cli.uninstallFailed", defaultValue: "Couldn't Uninstall Zenban CLI"),
                informativeText: error.localizedDescription,
                style: .warning
            )
        }
    ...
    private func installGhosttyConfigObserver() {
        ...
        ghosttyConfigObserver = NotificationCenter.default.addObserver(
            ...
            self?.refreshGhosttyConfigDrivenShortcuts()
        }
    }

    private func clearGhosttyConfigDrivenShortcuts() {
    ...
    private func refreshGhosttyConfigDrivenShortcuts(
        resolveShortcut: (String) -> StoredShortcut?
    ) {
        ghosttyGotoSplitLeftShortcut = resolveShortcut("goto_split:left")
    ...
    private func refreshGhosttyConfigDrivenShortcuts() {
        guard let config = GhosttyApp.shared.config else {
            clearGhosttyConfigDrivenShortcuts()
            return
        }

        refreshGhosttyConfigDrivenShortcuts { [self] action in
            storedShortcutFromGhosttyTrigger(
                ghostty_config_trigger(config, action, UInt(action.utf8.count))
            )
        }
    }
    ...
    private func matchArrowCapableShortcut(
        event: NSEvent,
    ...
    private func openNotificationInContext(_ context: MainWindowContext, tabId: UUID, surfaceId: UUID?, notificationId: UUID?) -> Bool {
        ...
        zenbanWorkspaceOpenHandler?(tabId)
...
final class MenuBarExtraController: NSObject, NSMenuDelegate {
    ...
    private let menu = NSMenu(title: "Zenban")
    ...
    private let quitItem = NSMenuItem(title: String(localized: "menu.quitCmux", defaultValue: "Quit Zenban"), action: nil, keyEquivalent: "")
        ...
        if let button = statusItem.button {
            ...
            button.toolTip = displayedUnreadCount == 0
                ...
                : displayedUnreadCount == 1
                    ? "Zenban: " + String(localized: "statusMenu.tooltip.unread.one", defaultValue: "1 unread notification")
                    : "Zenban: " + String(localized: "statusMenu.tooltip.unread.other", defaultValue: "\(displayedUnreadCount) unread notifications")
        }
    }
