...
class TerminalController {
        ...
        v2MainSync {
            if let ws = tabManager.tabs.first(where: { $0.id == wsId }) {
                if tabManager.tabs.count <= 1 {
                    AppDelegate.shared?.closeMainWindowContainingTabId(ws.id)
                } else {
    ...
    private func v2WorkspaceMoveToWindow(params: [String: Any]) -> V2CallResult {
        ...
        v2MainSync {
            guard let app = AppDelegate.shared,
                  app.tabManagerFor(tabId: wsId) != nil else {
                result = .err(code: "not_found", message: "Workspace not found", data: ["workspace_id": wsId.uuidString])
                return
            }
            guard app.mainWindow(for: windowId) != nil || app.tabManagerFor(windowId: windowId) != nil else {
                result = .err(code: "not_found", message: "Window not found", data: ["window_id": windowId.uuidString])
                return
            }
            guard app.moveWorkspaceToWindow(workspaceId: wsId, windowId: windowId, focus: focus) else {
                result = .err(
                    code: "invalid_state",
                    message: "Workspace move failed",
                    data: ["workspace_id": wsId.uuidString, "window_id": windowId.uuidString]
                )
                return
            }
            if focus, let manager = app.tabManagerFor(tabId: wsId) {
                setActiveTabManager(manager)
            }
            result = .ok([
                "workspace_id": wsId.uuidString,
                "workspace_ref": v2Ref(kind: .workspace, uuid: wsId),
    ...
    private func moveWorkspaceToWindow(_ args: String) -> String {
        ...
        v2MainSync {
            ...
            ok = app.moveWorkspaceToWindow(workspaceId: wsId, windowId: windowId, focus: focus)
            if ok, let manager = app.tabManagerFor(tabId: wsId) {
                setActiveTabManager(manager)
            }
        }
