...
actor TmuxSessionManager {
    ...
    /// Ensure tmux config exists in ~/.zenban/tmux.conf
    private func ensureConfigExists() {
        let zenbanDir = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".zenban")

        // Create ~/.zenban if needed
        try? FileManager.default.createDirectory(at: zenbanDir, withIntermediateDirectories: true)
    ...
    /// List all zenban-prefixed tmux sessions
    func listZenbanSessions() async -> [String] {
        guard let tmux = tmuxPath() else {
            return []
        }
    ...
    /// Kill all zenban-prefixed tmux sessions
    func killAllZenbanSessions() async {
